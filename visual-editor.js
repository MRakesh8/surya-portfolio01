(function() {
  ...
  function updateVideoBadges() {
    ...
    vids.forEach(function(vid) {
      var videoId = vid.dataset.videoId || vid.getAttribute('data-video-id');
      if (!videoId) return;
      if (!isNodeVisible(vid)) return;
      var rect = vid.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) return;
      var badge = document.createElement('div');
      badge.className = 've-video-badge';
      badge.textContent = '🎬 ' + videoId;
  ...
  // Auto-initialize badges and keep them updated on load, scroll, resize, and periodically
  if (document.readyState === 'complete') {
    updateVideoBadges();
  } else {
    window.addEventListener('load', updateVideoBadges);
  }
  window.addEventListener('resize', updateVideoBadges);
  window.addEventListener('scroll', updateVideoBadges);
  
  // Periodically check/re-draw in case layout shifts or elements get patched
  setInterval(updateVideoBadges, 1000);
